...
const MedievalHistory = () => {
  ...
  const getMedievalHistoryJson = async () => {
    try {
      const result = await instance.get("/getMedievalHistoryJsonFromBackend");
  ...
  const uploadFile = (file) => {
    ...
    instance
      .post("/uploadMedievalHistoryImagesToBackend", formData, {
        headers: {
      ...
      >
        {imageList.map((i) => (
          ...
          >
            ...
            <Image
              src={getImageFromBackend("medievalHistoryImages", i.imageApi)}
              alt="sample"
              height={180}
              width={200}
              style={{
