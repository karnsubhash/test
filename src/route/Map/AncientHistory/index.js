...
const AncientHistory = () => {
  ...
  const getAncientHistoryJson = async () => {
    try {
      const result = await instance.get("/getAncientHistoryJsonFromBackend");
  ...
  const uploadFile = (file) => {
    ...
    instance
      .post("/uploadAncientHistoryImagesToBackend", formData, {
        headers: {
      ...
      >
        {imageList.map((i) => (
          ...
          >
            ...
            <Image
              src={getImageFromBackend("ancientHistoryImages", i.imageApi)}
              alt="sample"
              height={180}
              width={200}
              style={{
